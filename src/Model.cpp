//
// Created by Gil Goldzweig on 2021-09-12.
//
...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene) {
    ...
    // 3. normal maps
    std::vector<TextureStruct> normalMaps = loadMaterialTextures(material, aiTextureType_HEIGHT, "texture_height");
    textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
    // 4. height maps
    std::vector<TextureStruct> heightMaps = loadMaterialTextures(material, aiTextureType_AMBIENT, "texture_normal");
...
std::vector<TextureStruct> Model::loadMaterialTextures(aiMaterial *mat, aiTextureType type, std::string typeName) {
    ...
    for (unsigned int i = 0; i < mat->GetTextureCount(type); i++) {
        ...
        for (auto loadedTexture: texturesLoaded) {
            if (strcmp(loadedTexture.path.data(), str.C_Str()) == 0) {
